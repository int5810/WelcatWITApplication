...
def create_xlsx_file(srcfile="subjectlist.csv", dstfile="subject.xlsx"):
    """概要(Summary line)
    ...
    with open(srcfile, "r") as fr:
        ...
        while line:
            ...
            csv_data.append([line_split[0], line_split[1]])
            line = fr.readline()
    # ワークブックと各種シートを作成
    wb = openpyxl.Workbook()
    if not __create_summary_sheet(wb, csv_data):
        return False
    if False:#not __create_print_sheet(wb, csv_data):
        return False
    # 保存
...
def __create_summary_sheet(wb, csv_data):
    ...
    for csv_row in csv_data:
        ...
        cell_b = ws.cell(row_idx, 2)
        cell_c = ws.cell(row_idx, 3)
        cell_a.value = csv_row[0]
        cell_b.value = csv_row[1]
        cell_c.value = "*" + csv_row[1] + "*"
        cell_a.alignment = normal_alignment
        cell_b.alignment = normal_alignment
        cell_c.alignment = Alignment(horizontal="left")
        cell_a.font = normal_font
        cell_b.font = normal_font
        cell_c.font = Font(name="CODE39", size=24)
        cell_b.number_format = openpyxl.styles.numbers.FORMAT_TEXT
        cell_c.number_format = openpyxl.styles.numbers.FORMAT_TEXT
